...
class JukeboxWindow(QWidget):
    ...
    def _build_icon(self):
        """A jukebox in the current skin's colours, carrying the game's emblem.

        The window is frameless, so this is all the task bar and the window
        switcher have to go on.
        """
        icon = QIcon()
        ...
        for side in (32, 48, 64, 128, 256):
            ...
            path.quadTo(body.right(), body.bottom(),
                        body.right() - radius, body.bottom())
            path.lineTo(body.left() + radius, body.bottom())
            path.quadTo(body.left(), body.bottom(),
                        body.left(), body.bottom() - radius)
            q.drawPath(path)
            ...
            for k in (-1, 1):
                ...
                cone = QRadialGradient(cx - r_sp * 0.32, cy - r_sp * 0.36,
                                       r_sp * 1.7)
                cone.setColorAt(0.0, self.accent.lighter(170))
                cone.setColorAt(0.55, self.accent)
                cone.setColorAt(1.0, self.accent.darker(210))
    ...
    def _paint_now_playing_text(self, p):
        ...
        # The same emblem as in the list, mirroring the play button on the
        # other side of the bar and turning off the same angle, so the two stay
        # in step.
        strip = self.r("now_playing")
        btn_h = self.r("play_button").height()
        side = max(16, int(btn_h * 0.92))
        badge = QRect(strip.right() + max(8, side // 5),
                      self.r("play_button").center().y() - side // 2, side, side)
        if self.current is not None:
            pm = self.sprite(TRACK_ICON.get(self.current.game, ""), side, side)
            if pm is not None:
                if self.player.state == "playing":
                    self._paint_spinning_emblem(p, badge, pm, self._spin)
                else:
                    p.setOpacity(0.55)
                    p.drawPixmap(badge, pm)
                    p.setOpacity(1.0)
